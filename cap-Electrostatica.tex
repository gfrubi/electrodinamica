...
\,C^2N^{-1}m^{-2}$. Para una discusión de los distintos sistemas de unidades usados en la teoría electromagnética, ver el apéndice \ref{appuni}.
...
\end{align}
de modo que define, dadas las componentes del campo $E_x(x,y,z)$, $E_y(x,y,z)$ y $E_y(x,y,z)$, un sistema de 3 ecuaciones diferenciales ordinarias acopladas, de primer orden, y en general no-lineal, para las incógnitas $x(\lambda),y(\lambda)$ y $z(\lambda)$.
...
tangencial a la superficie $S$), la condición (\ref{Etconst}) implica que
 \textit{las componentes tangenciales (2 componentes linealmente independientes) del campo eléctrico permanecen inalteradas
...
Como el potencial es una función continua, debemos tener que
\begin{equation}
\phi(R,\theta)=\alpha -E_0 R\cos\theta +
...
puede considerar el problema ``ficticio'' de encontrar las cargas ``imágenes''
...
En la sección anterior hemos situado el sistema coordenado de modo que la carga tenía posición $d\hat{x}$. Podemos reescribir el resultado para una carga puntual ubicada en una posición arbitraria $\vec{x}'$ en $V$. Manteniendo el origen del sistema de coordenadas en algún punto del plano, podemos entonces escribir la posición \textit{de la carga imagen} como
\begin{equation}
...
Como consecuencia, podemos usar esta función para encontrar una forma analítica para el potencial \textit{en una situación física más general}: el campo fuera de un conductor plano muy grande puesto a tierra, con una distribución de carga arbitraria de densidad $\rho(\vec{x})$, en el mismo dominio $V$ (es decir, fuera del conductor). La solución en este caso, ver la expresión \ref{solPoissonDirichlet} en la sección \ref{Green}, es de la forma
\begin{equation}
...
\begin{figure}[!h]
\centerline{\includegraphics[height=6cm]{fig/fig-metodo-imágenes-cilindros-01.pdf}}
...
\end{equation}
que es la ecuación de un cilindro (una circunferencia en el plano $xy$),
...
\end{equation}
\begin{figure}[!h]
\centerline{\includegraphics[height=6cm]{fig/fig-metodo-imágenes-cilindros-02.pdf}
\hspace{1cm}
\includegraphics[height=6cm]{fig/fig-metodo-imágenes-cilindros-03.pdf}}
\caption{Superficies equipotenciales y líneas de campo. Código Python de la figura de la derecha disponible \href{https://github.com/gfrubi/electrodinamica/blob/master/figuras-editables/fig-metodo-imágenes-cilindros-03.py}{aquí}.}
\label{ci04}
\end{figure}
...
\end{equation}
donde $\sigma$ es la densidad total de carga por unidad de superficie (en cada placa ubicada en $x=0$). Con esto, podemos evaluar la energía \eqref{UuE} (el campo eléctrico sólo es no nulo entre las placas, que encerrar un volumen $V=Ad$):
\begin{equation} 
...
En otras palabras, en general la polarización que se presentará en un medio es fruto de la  \textit{respuesta colectiva} del material a los campos externos.
 ...
 Vemos entonces que calcular $\vec{P}(\vec{x})$ desde ``primeros principios'' es en general difícil y requiere conocer los detalles de la estructura del material en estudio.
...
como \textit{electretos} (análogos eléctricos de los imanes permanentes), y 
...
E_jE_k+\cdots.}
\end{equation}
En general, si el material es \textit{inhomogéneo} (por ejemplo, si está formado por
...
gases, líquidos (excepto los, así llamados, \textit{cristales líquidos}), sólidos amorfos (plásticos, vidrio).

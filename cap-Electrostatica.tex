...
El potencial no es simplemente el de la carga puntual $\phi_{\rm puntual} = \dfrac{q}{4\pi\epsilon_0 r}$, puesto que la presencia de la carga $q$ induce una cierta cantidad de carga en la superficie del conductor. Luego, el potencial total es debido en parte a $q$ y en otra parte a la carga inducida $Q_{\rm ind}$. Pero, ¿c'omo calculamos el potencial si no conocemos cu'anta carga es inducida y c'omo esta se distribuye? 

Desde un punto de vista matem'atico, nuestro problema es resolver la ecuación de Poisson en la región $x>0$, con una carga puntual $q$ situada en $ (0,0,d) $ sujeta a las condiciones de contorno (a) $\phi = 0 $ en $x=0$ (pues el conductor est'a puesto a tierra) y (b) $V \rightarrow 0$ lejos de la carga (cuando $x^2 + y^2 + z^2 >> d^2$). Usando el m'etodo de las im'agenes resolvemos el problema equivalente obtenido al reemplazar el plano conductor por una carga ficticia (imagen) $q'$ ubicada en la posici'on $x=-d'<0$. El potencial de la configuraci'on de carga real m'as carga imagen es entonces
\begin{equation}
...
\end{equation}
Note que esta soluci'on un potencial distinto para la región $x < 0$. Sin embargo, esto no afecta la soluci'on del problema original, pues nuestra región de interés es la regi'on $x \ge 0$.
\begin{center}
\begin{figure}[H]
...
\begin{equation}
Q_{\rm ind} = \int \sigma(y,z) da ,
\end{equation}
que es m'as simple de calcular en coordenadas polares, lo que nos da que
\begin{equation}
Q_{\rm ind} = \int_0^{2\pi} \int_0^\infty \dfrac{-qd}{2\pi (r^2 + d^2)^{3/2}} \ r  dr  d\phi = \left. \dfrac{qd}{\sqrt{r^2 + d^2}} \right|_{0}^{\infty} = -q,
\end{equation}

...
Sabiendo que $J^\mu$ es un 4-vector, podemos analizar las propiedades de transformación de los campos eléctrico y magnético de modo que las ecuaciones de Maxwell inhomogéneas preserven su forma bajo TL's. Podemos escribir (\ref{inrho}) y (\ref{inj})  más explícitamente como:
\begin{eqnarray}
...
\subsection{Ecuacion de Maxwell homogénea y tensor dual*}
...
\vec{B}=\vec{\nabla}\times \vec{A}.
\end{equation}
Los potenciales no son funciones definidas unívocamente dada una configuración de campo electromagnético. Esto se manifiesta en la invariancia de los campos $\vec{E}$ y $\vec{B}$ bajo una transformación de gauge:
\begin{equation}
...
Si el 4-potencial satisface el gauge de Lorenz, la ecuación diferencial que éste debe satisfacer se reduce a la \textit{ecuación de onda inhomogénea}
\begin{equation}
\boxed{\square A^\mu=\mu_0 J^\mu .} \label{caj}
\end{equation}

En efecto, escribiendo las ecuaciones de Maxwell inhomogéneas (\ref{emihF}) en términos del 4-potencial, encontramos que
\begin{align}
\mu_0 J^\nu &=\partial_\mu F^{\mu\nu}  \\
...
\end{eqnarray}
Derivando (\ref{mre1}) y reemplazando en (\ref{mre1}) obtenemos
\begin{equation}
...
\end{equation}
que es equivalente a las ecuaciones de Maxwell inhomogéneas \eqref{coninhom}.
...
Escribiendo las componentes del tensor simétrico $T_{\rm em}^{\mu\nu}$ en términos
de las componentes de $\vec{E}$ y $\vec{B}$, encontramos que:
%En Gaussianas
%\begin{align}
%T_{\rm em}_0{}^0  & =\frac{1}{4\pi}\left(  F^{0\lambda}F_{\lambda0}+\frac
...
%\end{align}%
%\begin{align}
%T_{\rm em}_0{}^i  & =\frac{1}{4\pi}\left(  F^{i\lambda}F_{\lambda0}+\frac
...
%\end{align}%
%\begin{align}
%T_{\rm em}_j{}^i  & =\frac{1}{4\pi}\left(  F^{i\lambda}F_{\lambda j}+\frac
...
% Gaussianas
%\begin{eqnarray}
%u&:=&T_{\rm em}_0{}^0=\frac{1}{8\pi}\left( \vec{E}^2+\vec{B}^2\right),\\
%S^i&:=&cT_{\rm em}_0{}^i=\frac{c}{4\pi}\left(\vec{E}\times\vec{B}\right)^i=c^2\pi^i,\\
%T_j{}^i&:=&T_{\rm em}_j{}^i=\frac{1}{4\pi}\left(  E^i E^j +B^i B^j
%-\frac{1}{2}\left(  \vec{E}^2+\vec{B}^2\right)\delta_j^i\right),
%\end{eqnarray}
\begin{align}
u &:= T_{\rm em}^{00}=\frac{1}{2}\left(\varepsilon_0\vec{E}^2+\frac{1}{\mu_0}\vec{B}^2\right),\label{uTheta}\\
S^i &:= cT_{\rm em}^{0i}=\frac{1}{\mu_0}\left(\vec{E}\times\vec{B}\right)^i=c^2\pi^i, \label{STheta}\\
T^{ij} &:= T_{\rm em}^{ij}= \frac{1}{2}\left(\varepsilon_0\vec{E}^2+\frac{1}{\mu_0}\vec{B}^2\right)\delta^{ij}-\varepsilon_0 E^i E^j -\frac{1}{\mu_0}B^i B^j , \label{TTheta}
\end{align}
...
\partial_\nu T_{\rm em}^{0\nu}+F^0{}_\nu J^\nu=0
\end{equation}
que, luego de identificar las componentes de $T_{\rm em}$ de acuerdo a \eqref{uTheta}-\eqref{TTheta}, es equivalente a \eqref{econtEem}, es decir,
\begin{equation}
...
\nu}J^\nu\right]  dV \\
&=&\int_V \left[\frac{1}{c} \frac{\partial}{\partial t} T_{\rm em}^{\mu
0}+\partial_i T_{\rm em}^{\mu i}+\frac{1}{c} F^\mu_{\ \nu}J^\nu\right]  dV \\
&=&\int_V \left[\frac{1}{c} \frac{\partial}{\partial t} T_{\rm em}^{\mu
0}+\partial_i T_{\rm em}^{\mu i}+q\int d\tau\,  F^\mu_{\
\nu}u^\nu\,\delta^{(4)}\left(x^\mu-z^\mu(\tau)\right)\right]  dV \\
&=&\int_V \left[\frac{1}{c} \frac{\partial}{\partial t} T_{\rm em}^{\mu
0}+\partial_i T_{\rm em}^{\mu i}+c\int d\tau\,
\frac{dp^\mu}{d\tau}\,\delta^{(4)}\left(x^\mu-z^\mu(\tau)\right)\right]  dV \\
&=&\int_V \left[\frac{1}{c} \frac{\partial}{\partial t} T_{\rm em}^{\mu
0}+\partial_i T_{\rm em}^{\mu i}+c\int dt\,
\frac{dp^\mu}{dt}\,\delta^{(4)}\left(x^\mu-z^\mu(\tau)\right)\right]  dV
\\
&=&\frac{d}{dt}\frac{1}{c}\int_V  T_{\rm em}^{\mu 0}\,dV+\oint_{\partial V}T_{\rm em}^{\mu
i}dS_i+\int \frac{dp^\mu}{dt}\,\delta^{(4)}\left(x^\mu-z^\mu(\tau)\right)  d^4x
\\
&=&\frac{dP^\mu_{\rm campo}}{dt}+\oint_{\partial V}T_{\rm em}^{\mu
...
encontramos que el 4-moméntum total del campo y la carga es conservado
\begin{equation}

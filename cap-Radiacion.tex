...
\begin{equation}
\tilde{\Psi}(\vec{x},\omega):=\int_{-\infty}^\infty
\Psi(\vec{x},t)e^{-i\omega t}\,dt,
\end{equation}
\begin{equation}
\tilde{g}(\vec{x},\omega):=\int_{-\infty}^\infty
g(\vec{x},t)e^{-i\omega t}\,dt ,
\end{equation}
y entonces
\begin{equation}
g(\vec{x},t)=\frac{1}{2\pi}\int_{-\infty}^\infty
\tilde{g}(\vec{x},\omega)e^{+i\omega t}\,d\omega,
\end{equation}
\begin{equation}
\Psi(\vec{x},t)=\frac{1}{2\pi}\int_{-\infty}^\infty
\tilde{\Psi}(\vec{x},\omega)e^{+i\omega t}\,d\omega.
\end{equation}
...
\begin{equation}
\tilde{\Psi}(\vec{x},\omega)=-\int G(\vec{x},\vec{x}')\tilde{g}(\vec{x}',\omega)\,dV'.
\end{equation}
La función de Green $G(\vec{x},\vec{x}')$ puede considerarse como un caso particular de solución de (\ref{Hih}) correspondiente a una función fuente $\tilde{g}=\delta^{(3)}(\vec{x}-\vec{x}')$. En términos más físicos, la función de Green $G(\vec{x},\vec{x}')$ es proporcional al campo producido por una \textbf{fuente puntual}. En efecto, si $g(\vec{x},t)=f(t)\delta^{(3)}(\vec{x}-\vec{x}')$, entonces $\tilde{g}(\vec{x},\omega)=\tilde{f}(\omega)\delta^{(3)}(\vec{x}-\vec{x}')$, donde $\tilde{f}(\omega)$ es la transformada de Fourier de $f(t)$. De lo anterior, vemos que en este caso la solución de (\ref{Hih}) es de la forma $-\tilde{f}(\omega)G(\vec{x},\vec{x}')$.
...
transformada de Fourier $\tilde{\Psi}(\vec{x},\omega)$:
\begin{equation}
\tilde{\Psi}(\vec{x},\omega)=\frac{1}{4\pi}\int
\tilde{g}(\vec{x}',\omega)\frac{e^{\pm
...
\begin{eqnarray}
\Psi(\vec{x},t)&=&\frac{1}{2\pi}\frac{1}{4\pi}\int_{V'}\int_{-\infty}
^\infty \tilde{g}(\vec{x}',\omega)\frac{e^{\pm
ik|\vec{x}-\vec{x}'|}}{|\vec{x}-\vec{x}'|}e^{i\omega t}\,d\omega\,dV'  \\
&=&\frac{1}{2\pi}\frac{1}{4\pi}\int_{V'}\frac{1}{|\vec{x}-\vec{x}'|}\int_
{-\infty}^\infty \tilde{g}(\vec{x}',\omega)e^{i\omega t_{\pm}}\,d\omega \,dV' \\
...
$\vec{x}$ y en un instante $t$ dado \textit{en términos de la distribución de cargas y corrientes en tiempos anteriores a} $t$ (``solución causal''). En otras palabras, los potenciales (\ref{potret}) permiten \underline{\textbf{pre}}decir los campos producidos por un sistema de cargas dado. Por otro lado, la solución correspondiente al \textbf{tiempo avanzado} $t_{\rm av}:=t+{|\vec{x}-\vec{x}'|}/{c}\ge t$ relaciona los campos, en cada instante $t$, con la distribución de cargas en \textit{tiempos posteriores a }$t$ (``solución acausal'').

Para entender mejor estas relaciones, considere por simplicidad el caso en que las fuentes del sistema ($\vec{\rho}$ y $\vec{J}$) son no nulas sólo en un intervalo finito de tiempo $t\in (t_{\rm i},t_{\rm f})$. Esto ocurre, por ejemplo, en un condensador inicialmente descargado, que luego se carga al ser conectado a una batería, y que luego se descarga. Si $\Psi_{\rm i}(\vec{x},t)$ representa el campo ``inicial'', en tiempos $t<t_{\rm i}$, entonces podemos usar los potenciales retardados para \textit{pre}decir el campo en tiempos posteriores: $\Psi=\Psi_{\rm i}+\Psi_{\rm ret}$, ya que $\Psi_{\rm ret}(\vec{x},t)=0$ para $t<t_{\rm i}$. Por el contrario, si la información de la que disponemos es el campo $\Psi_{\rm f}$ en tiempos posteriores $t>t_{\rm f}$ entonces podemos usar los potenciales avanzados para ``\textit{retro}decir" los valores anteriores del campo: $\Psi=\Psi_{\rm f}+\Psi_{\rm av}$, ya que 
$\Psi_{\rm ret}(\vec{x},t)=0$ para $t>t_{\rm f}$. Note que tanto $\Psi_{\rm i}$ como $\Psi_{\rm f}$ satisfacen la ecuación de la onda homogénea.

Como en general nos interesa predecir los campos producidos por una distribución de cargas y corrientes, es decir, calcular cuanto valdrán los campos electromagnéticos en el futuro, nos concentraremos en los potenciales retardados. Las expresiones para estos potenciales son similares a aquellas válidas en los casos electrostático y magnetostático, ver \eqref{perho} y \eqref{defA} respectivamente, salvo que las fuentes dentro de la integral están evaluadas en el tiempo retardado, que depende tanto del tiempo como de la posición\footnote{Note que esta propiedad no es en general válida si los potenciales satisfacen gauges distintos al de Lorenz.}. Esto muestra que los potenciales retardados en un punto $\vec{x}$ y tiempo $t$ son una \textit{superposición de términos proporcionales a las fuentes ubicadas en \textit{cada punto} $\vec{x}'$, pero en tiempos anteriores} $t_{\rm ret}$, de forma tal que el \textit{evento}\footnote{es decir, un punto determinado del espacio y del tiempo.} definido por $(ct_{\rm ret},\vec{x}')$ tiene una \textit{relación tipo luz} con el evento $(ct,\vec{x})$. Esto último significa, como se desprende de (\ref{deftret}), que
...
La \textbf{sección total de scattering}, definida en general como la integral sobre el ángulo sólido de la sección diferencial, es llamada en este caso \textbf{sección transversal de Thomson}, y es dada por
\begin{equation}
\boxed{\sigma_{\rm T}=\frac{8\pi}{3}\left(\frac{q^2\mu}{4\pi m}\right)^2=\frac{8\pi}{3} r_q^2.}
\end{equation}

Para el caso de electrones, la sección transversal total de Thomson tiene el valor $\sigma_{\rm T}\approx 6.65\times 10^{-29}$ m$^2$.
...
de referencia en reposo con la carga blanco). Un cálculo cuántico del
scattering de fotones por partículas de carga $q$ y masa $m$ produce la sección
...
\begin{figure}[!h]
\centerline{\includegraphics[height=6cm]{fig/fig-Thomson-02.pdf}}
...
${d\sigma}/{d\Omega}$ en función de $\theta$ para distintos valores de $x:={\hbar\omega}/{mc^2}$.}
\label{thomson}
\end{figure}
...
\begin{equation}
\boxed{\left\langle\frac{dP_{m}}{d\Omega}\right\rangle (\hat{n}) :=\frac{2}{\mu c}\lim_{r\to\infty}r^2\left|\vec{E}_{m}\right|^2, \qquad m=1,2,3,\cdots,}
\end{equation}
de modo que
\begin{equation}
\boxed{\left\langle\frac{dP}{d\Omega}\right\rangle (\hat{n}) = \sum_{m=1}^{\infty}\left\langle\frac{dP_{m}}{d\Omega}\right\rangle (\hat{n}) .}
\end{equation}
La expresiones anteriores son válidas para cualquier fuente (compacta). Note además que la contribución correspondiente a $m=0$ será nula ya que corresponde a la parte del campo independiente del tiempo, que no tiene componente radiativa ya que decae al menos con una potencia $1/r^2$.

\subsubsection{Caso carga puntual}
Para el caso del campo producido por una carga puntual, la expresión (\ref{defAm}) nos permite entonces escribir:
\begin{eqnarray}
...
\begin{align}
\int_0^{T}\left.\frac{\hat{n}\times\left[\left(\hat{n}-\vec{\beta}\right)\times\vec{a}\right] }{\left(1-\vec{\beta}\cdot\hat{n}\right)^3}\right|_{\rm ret} e^{-i\omega_{m}t}dt &= \int_{-R/c}^{T-R/c}\left.\frac{\hat{n}\times\left[ \left( \hat{n}-\vec{\beta}\right)\times{\vec{a}}\right] }{\left( 1-\vec{\beta}\cdot\hat{n}\right)^3}\right|_{t'}e^{-i\omega_m (t'+R(t')/c)}\left(1-\vec{\beta}\cdot\hat{n}\right) dt' \\
...
\begin{equation}
R(t)  =r-\vec{z}(t) \cdot\hat{n}+O\left(r^{-1}\right). \label{expR} %aqui cambie
\end{equation}
Usando esta expansión, podemos escribir:
\begin{equation}
\int_0^{T}\left.\frac{\hat{n}\times\left[\left(\hat{n}-\vec{\beta}\right)\times\vec{a}\right] }{\left(1-\vec{\beta}\cdot\hat{n}\right)^3}\right|_{\rm ret} e^{-i\omega_{m}t}dt =e^{-i\frac{\omega_m r}{c}}\int_{-r/c}^{T-r/c}\frac{\hat{n}\times\left[ \left( \hat{n}-\vec{\beta}\right)\times{\vec{a}}\right] }{\left( 1-\vec{\beta}\cdot\hat{n}\right)^2}
...
\end{figure}
\begin{figure}[ht]
\centerline{\includegraphics[height=5cm]{fig/fig-mas-potencia-total-comparacion.pdf}\includegraphics[height=5cm]{fig/fig-mas-potencia-total-comparacion-logy.pdf}}
 \caption{Potencia promedio radiada para $m=1,\dots,5$, con parámetros de velocidad $\beta=0.5$ y  $\beta=0.1$, normalizadas respecto a la potencia total, $\left\langle P\right\rangle = \sum_m\left\langle P_m\right\rangle$.  Código Python \href{https://github.com/gfrubi/electrodinamica/blob/master/figuras-editables/fig-mas-potencia-total-comparacion.py}{aquí}.}
\label{TER3}
\end{figure}
...
Si la fuente (por ejemplo, la trayectoria $\vec{z}(t)$ en el caso de una carga puntual) no es periódica, los campos tampoco lo serán y en este caso la descomposición en oscilaciones armónicas del campo eléctrico toma la forma de una integral (o transformada) de Fourier $\tilde{\vec{E}}(\vec{x},\omega)$ del vector $\vec{E}(\vec{x},t)$, de modo que
\begin{equation}
...
=\frac{2\pi}{3}Z\left(\frac{d}{\lambda}\right)^2,
\end{equation}
donde hemos introducido la constante $Z:=\mu c=1/\varepsilon c=\sqrt{\mu/\varepsilon}$, que en el vacío asume el valor $Z_0\approx 377\,\Omega$. En este caso, $Z_0$ es llamada la \textit{impedancia del vacío}, o \textit{impedancia del espacio libre}.
...
%\end{equation}
Finalmente evaluando con $c\mu_0/4\pi\approx 30\, {\rm Nm\,A^{-2}s^{-1}}$, obtenemos
\begin{equation}
 \left\langle P\right\rangle
=36.564[\Omega]\,I_0^2.
\end{equation}

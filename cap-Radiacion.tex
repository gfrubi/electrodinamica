...
Primero introduciremos la \textit{transformada de Fourier temporal} de los campos involucrados (asumimos que estos campos son cuadrado-integrables),
\begin{equation}
...
\end{equation}
La funci'on de Green $G(\vec{x},\vec{x}')$ puede considerarse como un caso particular de soluci'on de (\ref{Hih}) correspondiente a una funci'on fuente $\bar{g}=\delta^{(3)}(\vec{x}-\vec{x}')$. En t'erminos m'as f'isicos, la funci'on de Green $G(\vec{x},\vec{x}')$ es proporcional al campo producido por una \textit{fuente puntual}. En efecto, si $g(\vec{x},t)=f(t)\delta^{(3)}(\vec{x}-\vec{x}')$, entonces $\bar{g}(\vec{x},\omega)=\bar{f}(\omega)\delta^{(3)}(\vec{x}-\vec{x}')$, donde $\bar{f}(\omega)$ es la transformada de Fourier de $f(t)$. De lo anterior, vemos que en este caso la soluci'on de (\ref{Hih}) es de la forma $-\bar{f}(\omega)G(\vec{x},\vec{x}')$.
...
\end{equation}
Los \textit{potenciales retardados} corresponden a la soluci'on con
\begin{equation}
\boxed{t_{\rm ret}:=t-\frac{|\vec{x}-\vec{x}'|}{c},} \label{deftret}
\end{equation}
que es llamado el \textit{tiempo retardado} (puesto que $t_{\rm ret}\le t$), de modo que
\begin{equation}
...
\end{equation}
donde $\dot{f}(\vec{x},t)$ denota la derivada partial respecto a la variable temporal de la que depende la funci'on $f(\vec{x},t)$, es decir $\dot{f}(\vec{x},t):=(\partial_t f)(\vec{x},t)$. Las relaciones anteriores son conocidas como \textit{ecuaciones de Jefimenko}\footnote{Oleg Jefimenko (1922-2009), f'isico estadounidense. Ver \url{http://en.wikipedia.org/wiki/Jefimenko}.}. Note que estas expresiones para los campos eléctrico y magnético difieren de aquellas que se encontrar'ian directamente la soluci'on de las ecuaciones inhomogéneas \eqref{EcOihE} y \eqref{EcOihB}. ?`Puede explicar por qu'e ocurre esto?.


\section{Potenciales de Li'enard-Wiechert}

Se conoce como \textit{potenciales de Li'enard}\footnote{Alfred Marie Li'enard: (1869-1958) f'isico franc'es.}\textit{-Wiechert}\footnote{Johann Emil Wiechert: (1861-1928) (geo)f'isico alem'an.} a los potenciales retardados generados por una \textit{carga puntual} $q$ \textit{que se mueve en forma conocida, pero arbitraria, con trayectoria} $\vec{z}(t)$. Ver figura (\ref{fig:LW}).
\begin{figure}[ht]
...
\end{equation}
para $|\vec\beta|<1$, con $\vec\beta:=\vec{v}/c$. Por ahora, asumiremos que siempre la velocidad de la part'icula es menor que la velocidad de la luz en el medio. Entonces, reemplazando (\ref{gp}) en (\ref{Ir}), se encuentra que
\begin{equation}
...
\label{R7}
\end{figure}
Sabemos que los campos el'ectrico y magn'etico est'an dados en t'erminos de derivadas de los potenciales de acuerdo a las expresiones \eqref{defA2} y \eqref{defphi2}. Para evaluar las derivadas involucradas es necesario tener en cuenta que los potenciales retardados dependen de la posici'on $\vec{x}$ y el tiempo $t$ expl'icitamente a trav'es de $\vec{R}=\vec{x}-\vec{z}(t')$ y $\vec{n}$, \textbf{pero adem'as impl'icitamente a trav'es del tiempo retardado} $t'=t'(\vec{x},t)$, que es soluci'on de \eqref{t'}.
...
\end{equation}
El campo magn'etico puede calcularse de forma similar, encontr'andose que \textbf{puede expresarse completamente en t'erminos del campo el'ectrico}:
\begin{equation}\marginnote{campo $\vec{B}$ carga $q$}
\boxed{\vec{B}(\vec{x},t)=\frac{1}{c}\left. \hat{n}\right|_{\rm
ret}\times\vec{E}(\vec{x},t) .}
\end{equation}
Este resultado muestra, por lo tanto, que \textbf{el campo magn'etico producido por una carga puntual es, en cada punto del espacio, perpendicular al campo el'ectrico y al vector que une la posici'on \underline{retardada} de la carga con el punto de observaci'on}.

Note que para distancias $r$ muy grandes (comparadas con las dimensiones de la
regi'on donde se encuentran las fuentes, que asumimos es una regi'on compacta) $R\approx r$ y entonces
...
carga acelera}. Por esto, $\vec{E}_{(2)}$ recibe el nombre de \textit{campo de
radiaci'on} o \textit{campo lejano}, mientras que $\vec{E}_{(1)}$ es llamado
\textit{campo cercano}.
...
\begin{figure}[!h]
\centerline{\includegraphics[height=5cm]{fig/fig-Larmor.pdf}\hspace{1cm}
\includegraphics[height=5cm]{fig/fig-Larmor-02.pdf}}
...
\end{equation}
\begin{figure}[H]
% \centerline{\psfig{file=fig/fig-lobulo-rel-01.pdf,height=8cm,angle=-90}}
\centerline{\psfig{file=fig/fig-carga-relativista.pdf,height=5cm}}
\caption{L'obulo de radiaci'on para una carga relativista acelerada. Asumimos
...
\end{equation}
Asumiendo que la velocidad de la carga es en todo momento mucho menor que la de la
...
-\omega t)}\right] .
\end{equation}
Asumiendo como condiciones iniciales $\vec{x}(0)=\vec{0}$ y $\vec{v}(0)=\vec{0}$, integramos esta ecuaci'on y obtenemos que la posici'on y velocidad de la carga tienen la forma
\begin{equation}
\vec{x}(t)
...
\begin{equation}
\frac{d\sigma}{d\Omega}=\left(\frac{q^2\mu}{4\pi m}\right)^2  \left(   %aqui cambie
...
\begin{equation}
\left\langle \frac{dP}{d\Omega}\right\rangle (\hat{n}) :=\frac{1}{T}\int_0%
^{T}\frac{dP}{d\Omega}(\hat{n},t)\,dt.
\end{equation}
...
&  =\frac{1}{\mu c}\lim_{r\to\infty}\frac{r^2}{T}\int_0^{T}\sum_{m}\vec{E}_{m}^*e^{-i\omega_{m}t}\cdot\sum_{m'}\vec{E}_{m'}e^{i\omega_{m'}t}dt\\ %aqui cambie
...
% \section{Problema 1}
%
% Utilizando los campos de Li\'{e}nerd-Wiechert, disc\'{u}tase la potencia
% media temporal radiada por unidad de 'angulo solido por una particula en
% movimiento no relativista de carga $e$, que se mueve arm'onicamente a lo
% largo del eje $z$, $z(t^{\prime })=a\cos (\omega _{0}t^{\prime })$
%
% \begin{description}
% \item[a.] Demu\'{e}strese que la potencia instan'anea radiada por unidad
% de 'angulo solido es
% \begin{equation}
% \frac{dP(t^{\prime })}{d\Omega }=\frac{e^{2}c\beta ^4}{16\pi
% ^{2}a^{2}\epsilon _{0}}\frac{\sin ^{2}\theta \cos ^{2}(\omega t^{\prime })}{%
% (1+\beta \cos \theta \sin \omega _{0}t^{\prime })^{5}}
% \end{equation}%
% en donde $\beta =a\omega _{0}/c$.
%
% \item[b.] Calculando el valor medio respecto del tiempo, demu\'{e}strese que
% la potencia media temporal radiada por unidad de 'angulo solido es
% \begin{equation}
% \frac{dP(t^{\prime })}{d\Omega }=\frac{e^{2}c\beta ^4}{128\pi
% ^{2}a^{2}\epsilon _{0}}\left[ \frac{4+\beta ^{2}\cos ^{2}\theta }{(1-\beta
% ^{2}\cos ^{2}\theta )^{7/2}}\right] \sin ^{2}\theta
% \end{equation}
%
% \item[c.] Repres\'{e}ntese de forma aproximada la distribuci'on para
% movimiento no relativista y relativista.
% \end{description}
%
% \textbf{Soluci'on:}
%
% \begin{description}
% \item[a.] Usando la ecuaci'on que nos da la potencia radiada por unidad
% de 'angulo s'olido, la cual es:
% \end{description}
%
% \begin{equation}
% \frac{dP}{d\Omega }=\frac{e^{2}}{16\pi ^{2}\epsilon _{0}c}\frac{\left\vert
% \widehat{n}\times \left[ \left( \widehat{n}-\beta \right) \times \dot{\beta}%
% \right] \right\vert ^{2}}{(1-\widehat{n}\cdot \beta )^{5}}
% \end{equation}
%
% Si consideramos que $\beta $ y $\dot{\beta}$ son paralelos y si consideramos
% que hay un 'angulo $\theta $ entre $\widehat{n}$ y $\beta $ entonces:%
% \begin{eqnarray}
% \frac{dP}{d\Omega } &=&\frac{e^{2}}{16\pi ^{2}\epsilon _{0}c}\frac{%
% \left\vert \widehat{n}\times \left[ \widehat{n}\times \dot{\beta}\right]
% \right\vert ^{2}}{(1-\widehat{n}\cdot \beta )^{5}} \\
% &=&\frac{e^{2}}{16\pi ^{2}\epsilon _{0}c}\frac{\dot{\beta}^{2}\sin
% ^{2}\theta }{(1-\widehat{n}\cdot \beta )^{5}}
% \end{eqnarray}
%
% Calculemos ahora una expresi'on para $\dot{\beta}$%
% \begin{eqnarray}
% z(t^{\prime }) &=&a\cos (\omega _{0}t^{\prime }) \\
% \ddot{z} &=&-\omega _{0}^{2}a\cos (\omega _{0}t^{\prime })=\dot{\beta}c
% \end{eqnarray}
%
% Reemplazando,%
% \begin{equation}
% \frac{dP}{d\Omega }=\frac{e^{2}\omega _{0}^4a^{2}}{16\pi ^{2}\epsilon
% _{0}c^{3}}\frac{\cos ^{2}(\omega _{0}t^{\prime })\sin ^{2}\theta }{(1+\frac{%
% \omega _{0}a}{c}\sin (\omega _{0}t^{\prime })\cos \theta )^{5}}
% \end{equation}
%
% Haciendo $\beta =a\omega _{0}/c$, llegamos a la expresi'on buscada,%
% \begin{equation}
% \frac{dP}{d\Omega }=\frac{e^{2}c\beta ^4}{16\pi ^{2}a^{2}\epsilon _{0}}%
% \frac{\sin ^{2}\theta \cos ^{2}(\omega _{0}t^{\prime })}{(1+\beta \cos
% \theta \sin \omega _{0}t^{\prime })^{5}}  \label{ec1}
% \end{equation}
%
% \begin{description}
% \item[b.] Ahora debemos calcular el promedio temporal de la ecuaci'on (%
% \ref{ec1}),%
% \begin{eqnarray}
% \langle \frac{dP}{d\Omega }\rangle &=&\frac{\omega _{0}}{2\pi }%
% \int_{0}^{2\pi /\omega _{0}}\frac{dP(t^{\prime })}{d\Omega }dt^{\prime }
% \nonumber \\
% &=&\frac{\omega _{0}e^{2}c\beta ^4\sin ^{2}\theta }{32\pi
% ^{3}a^{2}\epsilon _{0}}\int_{0}^{2\pi /\omega _{0}}\frac{\cos ^{2}(\omega
% _{0}t^{\prime })}{(1+\beta \cos \theta \sin \omega _{0}t^{\prime })^{5}}%
% dt^{\prime }  \nonumber \\
% &=&\frac{e^{2}c\beta ^4\sin ^{2}\theta }{32\pi ^{3}a^{2}\epsilon _{0}}%
% \int_{0}^{2\pi /\omega _{0}}\frac{\cos ^{2}(\omega _{0}t^{\prime })}{%
% (1+\beta \cos \theta \sin \omega _{0}t^{\prime })^{5}}d\left( \omega
% _{0}t^{\prime }\right)  \nonumber \\
% &=&\frac{e^{2}c\beta ^4\sin ^{2}\theta }{32\pi ^{3}a^{2}\epsilon _{0}}%
% \int_{0}^{2\pi }\frac{\cos ^{2}(\varphi )}{(1+\beta \cos \theta \sin \varphi
% )^{5}}d\varphi  \nonumber \\
% &=&\frac{e^{2}c\beta ^4\sin ^{2}\theta }{32\pi ^{3}a^{2}\epsilon _{0}}%
% \left\{ \frac{\pi }{4}\frac{4+\beta ^{2}\cos ^{2}\theta }{(1-\beta ^{2}\cos
% ^{2}\theta )^{7/2}}\right\}  \nonumber \\
% &=&\frac{e^{2}c\beta ^4\sin ^{2}\theta }{128\pi ^{2}a^{2}\epsilon _{0}}%
% \frac{\left( 4+\beta ^{2}\cos ^{2}\theta \right) }{(1-\beta ^{2}\cos
% ^{2}\theta )^{7/2}}  \label{ec2}
% \end{eqnarray}
%
% \item[c.] La velocidad es $\dot{z}=-\omega _{0}a\sin (\omega _{0}t^{\prime
% }) $ dado que la funci'on seno esta acotada, podemos simplemente hacer
% que $\left\vert \dot{z}\right\vert =\omega _{0}a.$
%
% \item Para el caso no relativista $\beta \ll 1$ por lo cual expandiendo en
% serie y tomando t\'{e}rminos proporcionales a $\beta ^4$, ya que son los
% de menor orden, (\ref{ec2}) nos queda proporcional a $\sin ^{2}\theta $, con
% lo cual la potencia por unidad de 'angulo queda de la siguiente forma:
% \end{description}
%
% Para el caso relativista $\beta \rightarrow 1$ \ y la expresi'on (\ref%
% {ec2}) queda%
% \begin{equation}
% \langle \frac{dP}{d\Omega }\rangle =\frac{e^{2}c\beta ^4\sin ^{2}\theta }{%
% 128\pi ^{2}a^{2}\epsilon _{0}}\frac{\left( 4+\cos ^{2}\theta \right) }{%
% (1-\cos ^{2}\theta )^{7/2}}
% \end{equation}
...
% \section{Problema 2}
%
% Utilizando la f'ormula de sumaci'on de Poisson o por otros
% procedimientos, demu\'{e}strese de forma expl'icita que si el movimiento
% de una part'icula radiante se repite con periodicidad $T$, el espectro
% continuo de frecuencias se convierte en un espectro discreto que contiene
% frecuencias que son multiplos enteros de la fundamental. Demu\'{e}strese que
% una expresi'on de la potencia radiada por unidad de 'angulo s'olido
% en cada m\'{u}ltiplo $m$ de la frecuencia fundamental $\omega _{0}=2\pi /T$
% es
% \begin{equation}
% \frac{dP_{m}}{d\Omega }=\frac{e^{2}\omega _{0}^4m^{2}}{32\pi ^4\epsilon
% _{0}c^{3}}\left\vert \int^{2\pi /\omega _{0}}\vec{v}(t)\times \hat{n}%
% e^{im\omega _{0}(t-\frac{\hat{n}\cdot \vec{x}(t)}{c})}dt\right\vert ^{2}
% \end{equation}
%
% \textbf{Soluci'on}
%
% La energ'ia radiada por unidad de 'angulo s'olido e intervalo de
% frecuencia por una carga que se mueve con trayectoria $\ r(t)$ en un punto
% lejano de observaci'on es
% \begin{equation}
% \frac{d^{2}I}{d\Omega d\omega }=\frac{e^{2}\omega ^{2}}{16\pi ^{3}c\epsilon
% _{0}}\left\vert \int_{-\infty }^{+\infty }n\times \left( n\times \beta
% \right) e^{i\omega \left\{ t-\frac{n\cdot r(t)}{c}\right\} }dt\right\vert
% ^{2}  \label{ec3}
% \end{equation}
%
% Si el movimiento es peri'odico, entonces el integrando tambi\'{e}n lo es,
% salvo una fase. Podemos entonces escribirlo como desarrollo de Fourier,%
% \begin{equation}
% n\times \left( n\times \beta \right) e^{-i\omega \frac{n\cdot r(t)}{c}%
% }=\sum_{m=-\infty }^{+\infty }B_{m}(\omega )e^{-im\omega _{0}t}  \label{ec4}
% \end{equation}
%
% Reescribiendo (\ref{ec3})%
% \begin{eqnarray*}
% \frac{d^{2}I}{d\Omega d\omega } &=&\frac{e^{2}\omega ^{2}}{16\pi
% ^{3}c\epsilon _{0}}\left\vert \int_{-\infty }^{+\infty }\sum_{m=-\infty
% }^{+\infty }B_{m}(\omega )e^{-im\omega _{0}t}e^{i\omega t}dt\right\vert ^{2}
% \\
% &=&\frac{e^{2}\omega ^{2}}{16\pi ^{3}c\epsilon _{0}}\int_{-\infty }^{+\infty
% }dt\int_{-\infty }^{+\infty }dt^{\prime }\sum_{m=-\infty }^{+\infty
% }\sum_{m^{\prime }=-\infty }^{+\infty }B_{m}(\omega )e^{-im\omega
% _{0}t}e^{i\omega t}B_{m^{\prime }}^{\ast }(\omega )e^{im^{\prime }\omega
% _{0}t^{\prime }}e^{-i\omega t^{\prime }} \\
% &=&\frac{e^{2}\omega ^{2}}{16\pi ^{3}c\epsilon _{0}}\sum_{m=-\infty
% }^{+\infty }\sum_{m^{\prime }=-\infty }^{+\infty }\int_{-\infty }^{+\infty
% }dtB_{m}(\omega )e^{-im\omega _{0}t}e^{i\omega t}B_{m^{\prime }}^{\ast
% }(\omega )\int_{-\infty }^{+\infty }dt^{\prime }e^{-i\left( \omega
% -m^{\prime }\omega _{0}\right) t^{\prime }} \\
% &=&\frac{e^{2}\omega ^{2}}{16\pi ^{3}c\epsilon _{0}}\sum_{m=-\infty
% }^{+\infty }\sum_{m^{\prime }=-\infty }^{+\infty }\int_{-\infty }^{+\infty
% }B_{m^{\prime }}^{\ast }(\omega )B_{m}(\omega )e^{-im\omega _{0}t}e^{i\omega
% t}2\pi \delta (\omega -m^{\prime }\omega _{0})dt
% \end{eqnarray*}
%
% Integrando en todas las frecuencias, cambiamos $\omega \rightarrow m^{\prime
% }\omega _{0}$ por acci'on de la delta de Dirac,%
% \begin{eqnarray*}
% \frac{dI}{d\Omega } &=&\int_{-\infty }^{+\infty }\frac{d^{2}I}{d\Omega
% d\omega }d\omega \\
% &=&\int_{-\infty }^{+\infty }\left\{ \frac{e^{2}\omega ^{2}}{16\pi
% ^{3}c\epsilon _{0}}\sum_{m=-\infty }^{+\infty }\sum_{m^{\prime }=-\infty
% }^{+\infty }\int_{-\infty }^{+\infty }B_{m^{\prime }}^{\ast }(\omega
% )B_{m}(\omega )e^{-im\omega _{0}t}e^{i\omega t}2\pi \delta (\omega
% -m^{\prime }\omega _{0})dt\right\} d\omega \\
% &=&\frac{e^{2}}{8\pi ^{2}c\epsilon _{0}}\sum_{m=-\infty }^{+\infty
% }\sum_{m^{\prime }=-\infty }^{+\infty }(m^{\prime }\omega
% _0)^{2}B_{m^{\prime }}^{\ast }(m^{\prime }\omega _0)B_{m}(m^{\prime
% }\omega _0)\int_{-\infty }^{+\infty }e^{-i\left( m-m^{\prime }\right)
% \omega _{0}t}dt
% \end{eqnarray*}
%
% Dado que el tiempo que estamos tomando va desde $-\infty $ a $+\infty $,
% entonces la energ'ia radiada ser'a, en general, infinita. Por esta raz%
% 'on nos restringimos solamente a un per'iodo de movimiento, es decir,
% $0\leq t\leq T,$
% \begin{eqnarray*}
% \frac{dI}{d\Omega } &=&\frac{e^{2}}{8\pi ^{2}c\epsilon _{0}}\sum_{m=-\infty
% }^{+\infty }\sum_{m^{\prime }=-\infty }^{+\infty }(m^{\prime }\omega
% _0)^{2}B_{m^{\prime }}^{\ast }(m^{\prime }\omega _0)B_{m}(m^{\prime
% }\omega _0)\int_{0}^{T}e^{-i\left( m-m^{\prime }\right) \omega _{0}t}dt \\
% &=&\frac{e^{2}}{8\pi ^{2}c\epsilon _{0}}\sum_{m=-\infty }^{+\infty
% }\sum_{m^{\prime }=-\infty }^{+\infty }(m^{\prime }\omega
% _0)^{2}B_{m^{\prime }}^{\ast }(m^{\prime }\omega _0)B_{m}(m^{\prime
% }\omega _0)T\delta _{m,m^{\prime }} \\
% &=&\frac{Te^{2}}{8\pi ^{2}c\epsilon _{0}}\sum_{m=-\infty }^{+\infty
% }(m\omega _0)^{2}B_{m}^{\ast }(m\omega _0)B_{m}(m\omega _0)
% \end{eqnarray*}
%
% Pero la potencia promedio irradiada es $dP=\frac{dI}{T},$ as'i%
% \begin{equation*}
% \frac{dP}{d\Omega }=\frac{e^{2}}{8\pi ^{2}c\epsilon _{0}}\sum_{m=-\infty
% }^{+\infty }(m\omega _0)^{2}\left\vert B_{m}(m\omega _0)\right\vert ^{2}
% \end{equation*}
%
% Para el $m$-\'{e}simo modo,%
% \begin{equation*}
% \frac{dP_{m}}{d\Omega }=\frac{m^{2}e^{2}\omega _{0}^{2}}{8\pi ^{2}c\epsilon
% _{0}}\left\vert B_{m}(m\omega _0)\right\vert ^{2}
% \end{equation*}
%
% Reescribiendo $B_{m}(m\omega _0)$ con ayuda de (\ref{ec4})%
% \begin{eqnarray*}
% n\times \left( n\times \beta \right) e^{-i\omega \frac{n\cdot r(t)}{c}}
% &=&\sum_{m=-\infty }^{+\infty }B_{m}(\omega )e^{-im\omega _{0}t} \\
% \int_{0}^{T}n\times \left( n\times \beta \right) e^{-i\omega \frac{n\cdot
% r(t)}{c}}e^{im^{\prime }\omega _{0}t}dt &=&\sum_{m=-\infty }^{+\infty
% }\int_{0}^{T}B_{m}(\omega )e^{-im\omega _{0}t}e^{im^{\prime }\omega _{0}t}dt
% \\
% &=&\sum_{m=-\infty }^{+\infty }B_{m}(\omega )T\delta _{m,m^{\prime }} \\
% B_{m}(\omega ) &=&\frac{1}{T}\int_{0}^{T}n\times \left( n\times \beta
% \right) e^{-i\omega \frac{n\cdot r(t)}{c}}e^{im\omega _{0}t}dt \\
% &=&\frac{\omega _{0}}{2\pi }\int_{0}^{2\pi /\omega _{0}}n\times \left(
% n\times \beta \right) e^{-i\omega \frac{n\cdot r(t)}{c}}e^{im\omega _{0}t}dt
% \end{eqnarray*}
%
% Finalmente,%
% \begin{eqnarray}
% \frac{dP_{m}}{d\Omega } &=&\frac{m^{2}e^{2}\omega _{0}^{2}}{8\pi
% ^{2}c\epsilon _{0}}\left\vert B_{m}(m\omega _0)\right\vert ^{2}  \notag \\
% &=&\frac{m^{2}e^{2}\omega _{0}^{2}}{8\pi ^{2}c\epsilon _{0}}\left\vert \frac{%
% \omega _{0}}{2\pi }\int_{0}^{2\pi /\omega _{0}}n\times \left( n\times \beta
% \right) e^{-im\omega _{0}\frac{n\cdot r(t)}{c}}e^{im\omega
% _{0}t}dt\right\vert ^{2}  \notag \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi ^4c^{3}\epsilon _{0}}\left\vert
% \int_{0}^{2\pi /\omega _{0}}n\times \left( n\times v\right) e^{-im\omega _{0}%
% \frac{n\cdot r(t)}{c}}e^{im\omega _{0}t}dt\right\vert ^{2} \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi ^4c^{3}\epsilon _{0}}\left\vert
% \int_{0}^{2\pi /\omega _{0}}\left( n\times v\right) e^{im\omega _{0}\left\{
% t-\frac{n\cdot r(t)}{c}\right\} }dt\right\vert ^{2}  \label{ec5}
% \end{eqnarray}
...
% \section{Problema 4}
%
% Una part'icula de carga $q$ se mueve en una trayectoria circular de
% radio $R$ situada en el plano $x-y$ con velocidad angular constante $\omega
% _{0}$
%
% \begin{description}
% \item
% \begin{description}
% \item[a.] Demu\'{e}strese que la expresi'on exacta para la distribuci'o%
% n angular de la frecuencia radiada en el multiplo $m$ de $\omega _{0}$ es
% \begin{equation}
% \frac{dP_{m}}{d\Omega }=\frac{e^{2}\omega _{0}^4R^{2}}{8\pi
% ^{2}c^{3}\epsilon _{0}}m^{2}\left[ \left( \frac{dJ_{m}(m\beta \sin \theta )}{%
% d(m\beta \sin \theta )}\right) ^{2}+\frac{\cot ^{2}\theta }{\beta ^{2}}%
% J_{m}^{2}(m\beta \sin \theta )\right]
% \end{equation}%
% en donde $\beta =\omega _{0}R/c$ y $J_{m}(x)$ es la funci'on de Bessel de
% orden $m$.
%
% \item[b.] Supongase movimiento no relativista y obt\'{e}ngase un resultado
% aproximado para $dP_{m}/d\Omega $. Demu\'{e}strese que en este caso l'imite
% se obtiene los resultados del problema 14.2.
%
% \item[c.] Sup'ongase que el movimiento es extremadamente relativista y
% obtengase los resultado hallados en el texto para una part'icula
% relativista en movimiento circular instant'aneo.
% \end{description}
% \end{description}
%
% \textbf{Soluci'on}
%
%
%  Las ecuaciones que gobiernan la trayectoria de la part'icula
% son%
% \begin{eqnarray}
% \vec{z}(t) &=&R\cos \left( \omega _0t\right) \hat{x}+R\sin \left( \omega
% _0t\right) \hat{y} \\
% \dot{r}(t) &=&\omega _0R\left\{ -\sin \left( \omega _0t\right) \hat{%
% \imath}+\cos \left( \omega _0t\right) \hat{y}\right\} \\
% \hat{n} &=&\cos \theta \hat{x}+\sin \theta \hat{z} \\
% \hat{n}\cdot \vec{z}(t) &=&R\{\cos \left( \omega _0t\right) \hat{x}\cdot
% \hat{n}+\sin (\omega _{0}t)\hat{y}\cdot \hat{n}\}  \notag \\
% &=&R\cos \left( \omega _0t\right) \cos \theta  \notag \\
% \hat{n}\times \left[ \hat{n}\times\vec{v}\right] &=&\omega _0R
% \left[ \sin \theta \sin \omega _{0}t\hat{x}-\cos \omega _{0}t\hat{y}%
% +\cos \theta \sin \theta \sin \omega _{0}t\hat{z}\right]
% \end{eqnarray}
%
%
%
% He escogido el vector $\hat{n}$ en el plano $xz$, debido a la simetr'ia
% azimutal del problema cuando integramos sobre toda la trayectoria, puesto
% que los l'obulos de radiaci'on de la part'icula forman un toro en
% el plano $xy.$
%
% Reemplazando estos datos en la ecuaci'on (\ref{ec5}), tenemos%
% \begin{eqnarray*}
% \frac{dP_{m}}{d\Omega } &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi
% ^4c^{3}\epsilon _{0}}\left\vert
% \begin{array}{c}
% \int_{0}^{2\pi /\omega _{0}}\frac{\omega _0R}{c}\left[ \sin ^{2}\theta
% \sin \omega _{0}t\hat{x}-\cos \omega _{0}t\hat{y}+\cos \theta \sin
% \theta \sin \omega _{0}t\hat{z}\right] \\
% \times \text{ }e^{im\omega _{0}\left\{ t-\frac{R\cos \left( \omega
% _0t\right) \cos \theta }{c}\right\} }dt%
% \end{array}%
% \right\vert ^{2} \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi ^4c^{3}\epsilon _{0}}\left(
% \frac{\omega _0R}{c}\right) ^{2}\left\vert
% \begin{array}{c}
% \sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega _0t\right) \hat{%
% \imath}e^{im\omega _{0}\left\{ t-\frac{R\cos \left( \omega _0t\right) \cos
% \theta }{c}\right\} }dt \\
% -\int_{0}^{2\pi /\omega _{0}}\cos \left( \omega _0t\right) \hat{y}%
% e^{im\omega _{0}\left\{ t-\frac{R\cos \left( \omega _0t\right) \cos \theta
% }{c}\right\} }dt \\
% +\cos \theta \sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega
% _0t\right) \hat{z}e^{im\omega _{0}\left\{ t-\frac{R\cos \left( \omega
% _0t\right) \cos \theta }{c}\right\} }dt%
% \end{array}%
% \right\vert ^{2} \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi ^4c^{3}\epsilon _{0}}\left(
% \frac{\omega _0R}{c}\right) ^{2}\left\vert I+J+K\right\vert ^{2}
% \end{eqnarray*}
%
% Definiendo $\beta =\frac{R\omega _{0}}{c}$%
% \begin{equation}
% \frac{dP_{m}}{d\Omega }=\frac{m^{2}e^{2}\omega _{0}^4}{32\pi
% ^4c^{3}\epsilon _{0}}\beta ^{2}\left\vert
% \begin{array}{c}
% \sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega _0t\right) \hat{%
% \imath}e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega _0t\right)
% \cos \theta }dt \\
% -\int_{0}^{2\pi /\omega _{0}}\cos \left( \omega _0t\right) \hat{y}%
% e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega _0t\right) \cos
% \theta }dt \\
% +\cos \theta \sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega
% _0t\right) \hat{z}e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega
% _0t\right) \cos \theta }dt%
% \end{array}%
% \right\vert ^{2}  \label{ec9}
% \end{equation}
%
% Calculemos ahora las integrales,%
% \begin{eqnarray}
% I &=&\sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega _0t\right)
% \hat{x}e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega
% _0t\right) \cos \theta }dt  \notag \\
% &=&\hat{x}\sin \theta \int_{0}^{2\pi /\omega _{0}}\frac{1}{2i}\left\{
% e^{i\omega _{0}t}-e^{-i\omega _{0}t}\right\} e^{im\omega _{0}t}e^{-im\beta
% _{0}\cos \left( \omega _0t\right) \cos \theta }dt  \notag \\
% &=&\hat{x}\sin \theta \int_{0}^{2\pi /\omega _{0}}\frac{1}{2i}\left\{
% e^{i\left( m+1\right) \omega _{0}t}-e^{i(m-1)\omega _{0}t}\right\}
% e^{-im\beta _{0}\cos \left( \omega _0t\right) \cos \theta }dt  \notag \\
% &=&\hat{x}\sin \theta \int_{0}^{2\pi /\omega _{0}}\frac{1}{2i}%
% \{e^{i\left( -m\beta _{0}\cos \left( \omega _0t\right) \cos \theta +\left(
% m+1\right) \omega _{0}t\right) }  \notag \\
% &&-e^{i\left( -m\beta _{0}\cos \left( \omega _0t\right) \cos \theta
% +\left( m-1\right) \omega _{0}t\right) }\}dt  \notag \\
% \omega _0t &\rightarrow &\phi \text{ \ \ \ \ \ \ \ \ \ \ }dt\rightarrow
% d\phi  \notag \\
% &=&\hat{x}\sin \theta \int_{0}^{2\pi }\frac{1}{2i}\{e^{i\left( -m\beta
% _{0}\cos \theta \cos \phi +\left( m+1\right) \phi \right) }-e^{i\left(
% -m\beta _{0}\cos \theta \cos \phi +\left( m-1\right) \phi \right) }\}d\phi
% \notag \\
% &=&\frac{\pi \sin \theta }{i}\{i^{m+1}J_{m+1}(m\beta _{0}\cos \theta
% )-i^{m-1}J_{m-1}(m\beta _{0}\cos \theta )\}\hat{x}  \notag \\
% &=&\pi \sin \theta \{i^{m+1}i^{-1}J_{m+1}(m\beta _{0}\cos \theta
% )-i^{m-1}i^{-1}J_{m-1}(m\beta _{0}\cos \theta )\}\hat{x}  \notag \\
% &=&i^{m}\pi \sin \theta \{J_{m+1}(m\beta _{0}\cos \theta )+J_{m-1}(m\beta
% _{0}\cos \theta )\}\hat{x}  \label{ec11}
% \end{eqnarray}
%
% Donde he usado la la representaci'on integral de las funciones de Bessel (%
% \ref{ec10}). La otras integrales nos quedan,%
% \begin{eqnarray}
% J &=&\hat{y}\int_{0}^{2\pi /\omega _{0}}\cos \left( \omega _0t\right)
% e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega _0t\right) \cos
% \theta }dt  \notag \\
% &=&\hat{y}\int_{0}^{2\pi /\omega _{0}}\frac{1}{2}\left\{ e^{i\omega
% _{0}t}+e^{-i\omega _{0}t}\right\} e^{im\omega _{0}t}e^{-im\beta _{0}\cos
% \left( \omega _0t\right) \cos \theta }dt  \notag \\
% &=&\hat{y}\int_{0}^{2\pi /\omega _{0}}\frac{1}{2}\left\{ e^{i\left(
% m+1\right) \omega _{0}t}+e^{i(m-1)\omega _{0}t}\right\} e^{-im\beta
% _{0}[\cos \left( \omega _0t\right) \cos \theta ]}dt  \notag \\
% \omega _0t &\rightarrow &\phi \text{ \ \ \ \ \ \ \ \ \ \ }dt\rightarrow
% d\phi  \notag \\
% &=&\hat{y}\frac{1}{2}\int_{0}^{2\pi }\left\{ e^{i\left( m+1\right) \phi
% }+e^{i(m-1)\phi }\right\} e^{-im\beta _{0}\cos \theta \cos \phi }d\phi
% \notag \\
% &=&\pi \{i^{m+1}J_{m+1}(m\beta _{0}\cos \theta )+i^{m-1}J_{m-1}(m\beta
% _{0}\cos \theta )\}\hat{y}  \notag \\
% &=&i^{m}\pi \{iJ_{m+1}(m\beta _{0}\cos \theta )-iJ_{m-1}(m\beta _{0}\cos
% \theta )\}\hat{y}  \notag \\
% &=&i^{m+1}\pi \{J_{m+1}(m\beta _{0}\cos \theta )-J_{m-1}(m\beta _{0}\cos
% \theta )\}\hat{y}
% \end{eqnarray}%
% \begin{eqnarray}
% K &=&\cos \theta \sin \theta \int_{0}^{2\pi /\omega _{0}}\sin \left( \omega
% _0t\right) \hat{z}e^{im\omega _{0}t}e^{-im\beta _{0}\cos \left( \omega
% _0t\right) \cos \theta }dt  \notag \\
% &=&\hat{z}\cos \theta \sin \theta \int_{0}^{2\pi /\omega _{0}}\frac{1}{2i}%
% \left\{ e^{i\omega _{0}t}-e^{-i\omega _{0}t}\right\} e^{im\omega
% _{0}t}e^{-im\beta _{0}\cos \left( \omega _0t\right) \cos \theta }dt  \notag
% \\
% &=&\frac{\pi \cos \theta \sin \theta }{i}\{i^{m+1}J_{m+1}(m\beta _{0}\cos
% \theta )-i^{m-1}J_{m-1}(m\beta _{0}\cos \theta )\}\hat{z}  \notag \\
% &=&i^{m}\pi \cos \theta \sin \theta \{J_{m+1}(m\beta _{0}\cos \theta
% )+J_{m-1}(m\beta _{0}\cos \theta )\}\hat{z}
% \end{eqnarray}
%
% An'alogo al procedimiento realizado en (\ref{ec11}). Usando la propiedad
% de ortonormalidad de los vectores bases, y las relaciones de recurrencia de
% las funciones de Bessel (\ref{ec13}) (\ref{ec12}), reemplazamos en la expresi%
% 'on (\ref{ec9}),%
% \begin{eqnarray*}
% \frac{dP_{m}}{d\Omega } &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi
% ^4c^{3}\epsilon _{0}}\beta ^{2}\left\vert
% \begin{array}{c}
% i^{m}\pi \sin \theta \frac{2m}{m\beta _{0}\cos \theta }J_{m}(m\beta _{0}\cos
% \theta )\hat{x} \\
% +i^{m+1}\pi 2\frac{dJ_{m}(m\beta _{0}\cos \theta )}{d(m\beta _{0}\cos \theta
% )}\hat{y} \\
% +i^{m}\pi \cos \theta \sin \theta \frac{2m}{m\beta _{0}\cos \theta }%
% J_{m}(m\beta _{0}\cos \theta )\hat{z}%
% \end{array}%
% \right\vert ^{2} \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi ^4c^{3}\epsilon _{0}}\beta ^{2}%
% \left[ \frac{4\pi ^{2}\sin ^{2}\theta }{\beta _{0}^{2}\cos ^{2}\theta }%
% J_{m}^{2}(m\beta _{0}\cos \theta )+4\pi ^{2}\left( \frac{dJ_{m}(m\beta
% _{0}\cos \theta )}{d(m\beta _{0}\cos \theta )}\right) ^{2}+\frac{4\pi
% ^{2}\sin ^{2}\theta }{\beta _{0}^{2}}J_{m}^{2}(m\beta _{0}\cos \theta )%
% \right]
% \end{eqnarray*}
%
% Recordemos por la figura 3, que el 'angulo $\theta $ lo forma el vector
% normal con el eje $x.$Si ahora cambiamos el par'ametro tal que el 'a%
% ngulo sea el que se forma entre el vector normal y el eje $z$, entonces $%
% \theta ^{\prime }=\frac{\pi }{2}-\theta $ , por lo cual la expresi'on
% anterior en t\'{e}rminos de $\theta ^{\prime }$ es%
% \begin{eqnarray}
% \frac{dP_{m}}{d\Omega } &=&\frac{m^{2}e^{2}\omega _{0}^4}{32\pi
% ^4c^{3}\epsilon _{0}}\beta _{0}^{2}\left[ \frac{4\pi ^{2}\cos ^{2}\theta }{%
% \beta _{0}^{2}\sin ^{2}\theta }J_{m}^{2}(m\beta _{0}\sin \theta )+4\pi
% ^{2}\left( \frac{dJ_{m}(m\beta _{0}\sin \theta )}{d(m\beta _{0}\sin \theta )}%
% \right) ^{2}+\frac{4\pi ^{2}\cos ^{2}\theta }{\beta _{0}^{2}}%
% J_{m}^{2}(m\beta _{0}\sin \theta )\right]  \notag \\
% &=&\frac{m^{2}e^{2}\omega _{0}^44\pi ^{2}}{32\pi ^4c^{3}\epsilon _{0}}%
% \beta _{0}^{2}\left[ \frac{\cos ^{2}\theta }{\beta _{0}^{2}}J_{m}^{2}(m\beta
% _{0}\sin \theta )\left\{ 1+\csc ^{2}\theta \right\} +\left( \frac{%
% dJ_{m}(m\beta _{0}\sin \theta )}{d(m\beta _{0}\sin \theta )}\right) ^{2}%
% \right]  \notag \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{38\pi ^{2}c^{3}\epsilon _{0}}\beta
% _{0}^{2}\left[ \frac{1}{\beta _{0}^{2}}J_{m}^{2}(m\beta _{0}\sin \theta
% )\left\{ 2\cos ^{2}\theta +\cos ^{2}\theta \cot ^{2}\theta \right\} +\left(
% \frac{dJ_{m}(m\beta _{0}\sin \theta )}{d(m\beta _{0}\sin \theta )}\right)
% ^{2}\right]  \notag \\
% &=&\frac{m^{2}e^{2}\omega _{0}^4}{38\pi ^{2}c^{3}\epsilon _{0}}\beta
% _{0}^{2}\left[ \frac{1}{\beta _{0}^{2}}J_{m}^{2}(m\beta _{0}\sin \theta
% )\cot ^{2}\theta +\left( \frac{dJ_{m}(m\beta _{0}\sin \theta )}{d(m\beta
% _{0}\sin \theta )}\right) ^{2}\right]
% \end{eqnarray}
%
% En el caso no relativista $\beta _{0}\ll 1$ por lo cual el
% argumento de las funciones de Bessel es mucho menor que 1 y podemos usar la
% aproximaci'on,%
% \begin{equation}
% J_{m}^{2}(x)\approx \frac{1}{\left( m!\right) ^{2}}\left( \frac{x}{2}\right)
% ^{2m}\text{ \ \ \ \ \ \ \ \ }x\ll 1
% \end{equation}
%
%
% As'i tenemos que para $m$ grandes las contribuciones a la potencia son
% muy peque\~nas, por lo que consideraremos el primer arm'onico $m=1,$
% luego%
% \begin{eqnarray*}
% J_{1}^{2}(\beta _{0}\sin \theta ) &\approx &\frac{\beta _{0}^{2}\sin
% ^{2}\theta }{4} \\
% \frac{dJ_{m}(m\beta _{0}\sin \theta )}{d(m\beta _{0}\sin \theta )} &=&\frac{1%
% }{2}\left[ J_{m-1}(m\beta _{0}\sin \theta )-J_{m+1}(m\beta _{0}\sin \theta )%
% \right]  \\
% \frac{dJ_{1}(\beta _{0}\sin \theta )}{d(\beta _{0}\sin \theta )} &=&\frac{1}{%
% 2}\left[ J_{0}(\beta _{0}\sin \theta )-J_2(\beta _{0}\sin \theta )\right]
% \\
% &\approx &\frac{1}{2}\left[ \frac{1}{2}-\frac{1}{4}\frac{\beta _{0}^4\sin
% ^4\theta }{16}\right]  \\
% &\approx &\frac{1}{4}-\frac{\beta _{0}^4\sin ^4\theta }{64} \\
% \left( \frac{dJ_{1}(\beta _{0}\sin \theta )}{d(\beta _{0}\sin \theta )}%
% \right) ^{2} &\approx &\frac{1}{16}+O(\beta _{0}^{8})
% \end{eqnarray*}
%
% Reemplazando tenemos:%
% \begin{eqnarray*}
% \frac{dP_{1}}{d\Omega } &\approx &\frac{e^{2}\omega _{0}^4R^{2}}{8\pi
% ^{2}c^{3}\epsilon _{0}}\left[ \frac{1}{16}+\frac{\cot ^{2}\theta }{\beta ^{2}%
% }\frac{\beta _{0}^{2}\sin ^{2}\theta }{4}\right] \\
% &\approx &\frac{e^{2}\omega _{0}^4R^{2}}{8\pi ^{2}c^{3}\epsilon _{0}}\left[
% \frac{1}{16}+\frac{\cos ^{2}\theta }{4}\right] \\
% &\approx &\frac{e^{2}\omega _{0}^4R^{2}}{32\pi ^{2}c^{3}\epsilon _{0}}\left[
% \frac{1+\cos ^{2}\theta }{16}\right]
% \end{eqnarray*}
...
%\end{equation}
Como asumimos que la corriente var'ia arm'onicamente en el tiempo, tendremos que $\langle\ddot{I}^ 2\rangle=\omega^4\langle I^ 2\rangle$ y, por lo tanto, la resistencia de radiaci'on resulta ser
\begin{equation}
...
\end{equation}
Por lo tanto, como hemos asumido que $a\ll\lambda/2\pi$, encontramos que  $R_{\rm rad}^{\vec\mu}\ll R_{\rm rad}^{\vec{p}}$. En este sentido un peque\~no condensador rad'ia m'as ``eficientemente'' que una espira de similar tama\~no, alimentada con la misma corriente alterna.
...
Consideremos una \textit{antena lineal} de largo $d$, alimentada en su punto medio por una fuente alterna de frecuencia angular $\omega$, orientada a lo largo del eje $z$. Asumiremos que el perfil de corriente en la antena es de la forma
\begin{equation}
